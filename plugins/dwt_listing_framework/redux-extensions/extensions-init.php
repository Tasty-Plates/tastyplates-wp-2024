...
Redux::setExtensions( 'dwt_listing_options', dirname( __FILE__ ) . '/extensions/' );
...
if ( !function_exists( 'dwt_listing_framework_description_text' ) ) {
    function dwt_listing_framework_description_text( $description ) {
		//get ser detials
		...
		if ( phpversion() < $php_version)
		{
			$active_clr = 'bad-req';
			$icon = 'no';
			$msg = '<p>You have outdated PHP version installed on your server. PHP version 7.0 or higher is required to make sure DWT Listing Theme and all required plugins work correctly.</p> <a href="https://www.php.net/supported-versions.php"> Click here to read more details </a>';
		}
		//max execution
		...
		if ( $server_max_execution_time < $min_execution_time)
		{
			$ok_clr = 'bad-req';
			$e_icon = 'no';
			$e_msg = '<p>Your server has limited max_execution_time. We recommend you to increase it to 2000 (seconds) or more to make sure demo import will have enough time to load all demo content & images</p>';
		}
		//max file size
        ...
        $message = '<p>'. esc_html__( 'Best if used on new WordPress install & this theme requires PHP version 7.0+', 'dwt-listing-framework' ) .'</p>';
        $message .= '<p>'. esc_html__( 'Images are for demo purpose only.', 'dwt-listing-framework' ) .'</p>';
        $message .= '
    ...
    add_filter( 'wbc_importer_description', 'dwt_listing_framework_description_text', 10 );
}