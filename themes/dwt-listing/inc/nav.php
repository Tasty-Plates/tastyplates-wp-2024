...
if (!function_exists('dwt_listing_getMenuItemsData')) {

    function dwt_listing_getMenuItemsData($item_id='', $item='', $menu_level = 1, $show_indicator = true)
    {
        $item_data = array();
        $arr = array();
        $angleAerrow = '';
        ...
        if (isset($dwt_listing_theme['sb_rtl']) && $dwt_listing_theme['sb_rtl']) {
            $angle = 'fa-angle-left';
        }
        if ($menu_level == 2) {
            $icon = ($menu_icon != "") ? ' <i class="' . $menu_icon . '"></i>' : '';
...
if (!function_exists('dwt_listing_themeMenu')) {

    function dwt_listing_themeMenu($theme_location='')
    {
            ...
            if (isset($menu->term_id)) {
                ...
                foreach ($menu_items as $item) {
                    if ($item->menu_item_parent == 0) {
                        $level1 = 1;
                        $menuItems1 = dwt_listing_getMenuItemsData($item->ID, $item, 1);
                        $menu_html .= '<li>';
                        ...
                        foreach ($menu_items as $sub_item) {
                            if ($item->ID == $sub_item->menu_item_parent) {
                                ...
                                if ($mainMenuClass == 'drop-down' || $mainMenuClass == 'drop-down-multilevel') {
                                    $menuItems2 = dwt_listing_getMenuItemsData($sub_item->ID, $sub_item, 2);
                                    if ($level1 == 1) {
                                    ...
                                    foreach ($menu_items as $sub_sub_item) {
                                        if ($sub_item->ID == $sub_sub_item->menu_item_parent) {
                                            ...
                                            $menuItems3 = dwt_listing_getMenuItemsData($sub_sub_item->ID, $sub_sub_item, 3, $show_indicator);
                                            $mega_menu_html .= '<li>';
                ...
                $menu_html .= '<li><a href="' . esc_url(home_url()) . '">' . __("Home", "dwt-listing") . '</a></li>';
            }
        } else {
            $menu_html .= '<li><a href="' . esc_url(home_url()) . '">' . __("Home", "dwt-listing") . '</a></li>';
        }
        //echo($menu_html);
        echo dwt_listing_wpml_nav_menu_callback($menu_html, $theme_location);
    }

}


add_action('init', array('dwt_listing_Nav_Menu_Item_Custom_Fields', 'setup'));
if (!class_exists('dwt_listing_Nav_Menu_Item_Custom_Fields')) {

    class dwt_listing_Nav_Menu_Item_Custom_Fields
    {
        ...
        static function setup()
        {
            if (!is_admin())
                return;
            $new_fields = apply_filters('dwt_listing_nav_menu_item_additional_fields', array());
            if (empty($new_fields))
                return;

            self::$options['fields'] = self::get_fields_schema($new_fields);
            add_filter('wp_edit_nav_menu_walker', function () {
                return 'dwt_listing_Walker_Nav_Menu_Edit';
            });
            ...
            foreach (self::$options['fields'] as $field) {
                ...
                if ($field['input_type'] == 'only_text') {
                    ...
                    if ($field['name'] == 'is_megamenu_class') {
                        ...
                        $vals = array(__("Select Option", "dwt-listing") => "", __("Simple Menu", "dwt-listing") => "simple-menu", __("Mega Menu", "dwt-listing") => "mega-menu");
                    ...
                    if ($field['name'] == 'is_megamenu') {
                        ...
                        for ($size = 12; $size >= 1; $size--) {
                            ...
                            $ar .= '<option value="grid-col-' . esc_attr($size) . '" ' . $selected . '>' . __("Column size ", "dwt-listing") . $size . '</option>';
                        }
                    }
                    ...
                    if ($field['name'] == 'menu_open_side') {
                        $savedValue = get_post_meta($item->ID, self::get_menu_item_postmeta_key($field['name']), true);
                        $vals = array(__("Select Option", "dwt-listing") => "", __("Right Side", "dwt-listing") => "right-side", __("Left Side", "dwt-listing") => "left-side");
                        $ar = '';
                        foreach ($vals as $val => $name) {
                            $selected = ($savedValue == $name) ? 'selected="selected"' : '';
                            $ar .= '<option value="' . esc_attr($name) . '" ' . $selected . '>' . esc_html($val) . '</option>';
                    ...
                    if ($field['name'] == 'menu_simple_offset') {
                        $savedValue1 = get_post_meta($item->ID, self::get_menu_item_postmeta_key($field['name']), true);
                        $ar = '';
                        $ar .= '<option value="">' . __("Select Option - (No Offset)", "dwt-listing") . '</option>';
                        for ($size = 12; $size >= 1; $size--) {
                            $selected = ('offset-' . $size == $savedValue1) ? 'selected="selected"' : '';
                            $ar .= '<option value="offset-' . esc_attr($size) . '" ' . $selected . '>' . __("Column Offset ", "dwt-listing") . $size . '</option>';
                        }
                    }
                    ...
                    if ($field['name'] == 'menu_vertical_offset') {
                        ...
                        for ($size = 12; $size >= 1; $size--) {
                            ...
                            $ar .= '<option value="offset-' . esc_attr($size) . '-vertical" ' . $selected . '>' . __("Vertical Offset", "dwt-listing") . $size . '</option>';
                        }
                    }
...
if (!class_exists('dwt_listing_Walker_Nav_Menu_Edit')) {

    class dwt_listing_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit
    {
        ...
        function start_el(&$output='', $item='', $depth = 0, $args = array(), $id = 0)
        {
            ...
            if ($new_fields = dwt_listing_Nav_Menu_Item_Custom_Fields::get_field($item, $depth, $args)) {
...
// Somewhere in config...
add_filter('dwt_listing_nav_menu_item_additional_fields', 'opportunitiesMenu_item_additional_fields');
if (!function_exists('opportunitiesMenu_item_additional_fields')) {
    ...
    function opportunitiesMenu_item_additional_fields($fields='')
    {
        ...
        $fields['some_text'] = array(
            'name' => 'some_text',
            'label' => '*' . __('If you want to hide column name in mega menu just write {HideMe} in  Navigation Label', 'dwt-listing'),
        ...
        $fields['color'] = array(
            'name' => 'color',
            'label' => __('Lebel Color', 'dwt-listing'),
            'container_class' => 'link-color',
            'input_type' => 'color',
        );
        $fields['color_text'] = array(
            'name' => 'color_text',
            'label' => __('Lebel Text', 'dwt-listing'),
            'container_class' => 'link-color-text',
            'placeholder' => __('Enter some text', 'dwt-listing'),
            'input_type' => 'text',
        );
        $fields['megamenu_class'] = array(
            'name' => 'is_megamenu_class',
            'label' => __('This select option is only for 1st level menu', 'dwt-listing'),
            'container_class' => 'megamenu-classes',
            'input_type' => 'select',
        );
        $fields['megamenu'] = array(
            'name' => 'is_megamenu',
            'label' => __('In parent add bigger value and in 2nd level add small value', 'dwt-listing'),
            'container_class' => 'megamenu-checkbox',
            'input_type' => 'select',
        );
        $fields['menu_icons'] = array(
            'name' => 'menu_icons',
            'label' => __('Menu Icons (Font Awesome Icons). ', 'dwt-listing') . 'http://fontawesome.io/icons/',
        ...
        $fields['menu_open_side'] = array(
            'name' => 'menu_open_side',
            'label' => __('Select Options - For simple menu only', 'dwt-listing'),
        ...
        $fields['menu_simple_offset'] = array(
            ...
            'label' => __('Select Options (For mega menu 1st level menu only)', 'dwt-listing'),
...
if (!function_exists('dwt_listing_wpml_nav_menu_callback')) {

    function dwt_listing_wpml_nav_menu_callback($menu_html = '', $theme_location='')
    {

        $dwt_wpml_menu = false;
        if (class_exists('Redux')) {
            $dwt_wpml_menu = Redux::get_option('dwt_listing_options', 'dwt_listing_menu_lang_switch');
        }
        if (function_exists('icl_object_id') && $dwt_wpml_menu) {
            if (($theme_location) && ($locations = get_nav_menu_locations()) && isset($locations[$theme_location])) {
                $menu = get_term($locations[$theme_location], 'nav_menu');
                if (isset($menu->term_id)) {
                    $menu_items = wp_get_nav_menu_items($menu->term_id);
